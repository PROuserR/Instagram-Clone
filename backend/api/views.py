...
def list_users(request):
    ...
    for user in users_serializer.data:
        ...
        final_data.append({'id':user['id'], 'username':user['username'] , 'profile_picture': str(profile.image)})
...
def list_followed_users(request, my_id):
    ...
    for following_id in following_ids:
        ...
        final_data.append({'id':user.id, 'username':user.username, 'profile_picture':str(profile.image), 'last_seen':user.last_login})
    ...
    for post in serializer.data:
        ...
        for key in post.keys():
            ...
            if len(post['likes'])> 0:
                ...
                data['liker_id'] = Like.objects.get(id=post["likes"][0]).liker.id
        final_data.append(data)
...
def create_post(request):
    ...
    data = request.POST
    post = Post.objects.create(caption=data['caption'], owner=User.objects.get(id=request.user.id))
...
def get_activites(request):
    activites = Activity.objects.filter(influnced=request.user.id).order_by('date_added')
...
def get_users_stories(request):
    ...
    order_by_date_added = lambda data: data.date_added.strftime('%Y-%m-%d-%H-%M-%S')
    stories = [ Story.objects.get(owner=id) for id in following if Story.objects.filter(owner=id) ]
