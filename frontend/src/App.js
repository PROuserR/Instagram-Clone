...
function App() {
  ...
  const [onlineUsersSocket, setOnlineUsersSocket] = useState(new WebSocket('ws://127.0.0.1:8000/ws/online_users'))
  ...
  useEffect(() => {
    if (sendFlag && onlineUsersSocket.readyState === 1) {
      onlineUsersSocket.send(JSON.stringify({
        'users': { 'id': localStorage.getItem('my_id'), 'username': localStorage.getItem('my_username'), 'profile_picture': localStorage.getItem('my_profile_picture') },
      }))
  ...
  if (startAppFlag)
    return (
      <main className="bg-white w-screen text-sm mx-auto max-w-4xl h-screen overflow-auto dark:text-white dark:bg-black">
        <OnlineUsersContext.Provider value={[onlineUsersContext, setOnlineUsersContext]}>
    ...
    return (
      <section className='fixed top-0 left-0 flex flex-col w-screen h-screen bg-white dark:bg-black z-20'>
        ...
        <div className='mx-auto text-lg text-black dark:text-white font-mono'>from</div>
        <div className='mx-auto text-lg font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-pink-500 to-yellow-500'>Rami</div>
      </section>
    )
