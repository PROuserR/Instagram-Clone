...
const MyProfilePage = () => {
  ...
  const taggedGridEmptyContent = <section className='flex flex-col items-center text-center space-y-2'>
    ...
    <div className='text-gray-400 w-64'>When people tag you in photos and videos, they'll appear here</div>
    <div className='pb-20'></div>
  </section>

  const logoutUser = async () => {
    await fetch('http://127.0.0.1:8000/auth/logout_user/')
    await fetch(`http://127.0.0.1:8000/auth/user_last_login_update/${localStorage.getItem('my_id')}`)
    nav('/login')
  }
  ...
  const toggleDiscoverUsers = () => {
    if (discoverUsers.length > 0)
      ...
      setDiscoverUsers(discoverUsersCopy)
  }
      ...
      {profile.map((profile, index) => (
        <div key={index}>
          <section className='flex items-center px-2'>
            ...
            <div className='ml-2 text-xs' onClick={() => setLoginMenuFlag(!loginMenuFlag)}>\/</div>
            <div className='flex ml-auto space-x-4'>
              <AddIcon className='w-7 my-3 rounded-md cursor-pointer' onClick={() => nav('/addPost')} />
            </div>
            <div className='flex ml-4 space-x-4'>
              <ListIcon onClick={() => setSettingsMenuFlag(!settingsMenuFlag)} />
            </div>
            ...
            {loginMenuFlag ?
              <div className='flex flex-col space-y-4 p-4 pt-6 w-screen max-w-4xl bg-white dark:bg-zinc-900 fixed left-0 bottom-10 xl:left-72 z-10'>
                <button className='font-extrabold absolute top-0 left-44 text-zinc-400' onClick={() => setLoginMenuFlag(!loginMenuFlag)}>
                  ______
                </button>
                <div className='flex items-center space-x-2'>
                  <img className='rounded-full w-12' src={`http://127.0.0.1:8000${profile.image}`} />
                  <span className='text-lg font-semibold'>{profile.username}</span>
                ...
                <div className='flex items-center space-x-2'>
                  <div className='border border-gray-500 rounded-full p-1.5'>
                    <PlusIcon className='w-8 h-8 cursor-pointer' onClick={logoutUser} />
                  </div>
                  <span className='text-lg font-semibold' onClick={logoutUser}>Add account</span>
                </div>
              </div>
              : null}
            ...
            {settingsMenuFlag ?
              <div className='p-4 pt-6 w-screen max-w-4xl bg-white dark:bg-zinc-900 fixed left-0 bottom-10 xl:left-72 z-10'>
                <div className='flex w-full items-center space-x-3' onClick={() => nav('/saved-posts')}>
                  <SaveIcon />
          ...
          <section className='p-3 flex items-center'>
            {localStorage.getItem('my_profile_picture') ? <div className='w-auto bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 rounded-full p-0.5'><div className='w-auto h-auto bg-white dark:bg-black rounded-full p-1'><img className='w-20 h-20 aspect-square rounded-full' src={!localStorage.getItem('my_profile_picture').includes('platform-lookaside') ? `http://127.0.0.1:8000/media/${localStorage.getItem('my_profile_picture')}` : localStorage.getItem('my_profile_picture')} /></div></div>
              : <div className='w-auto h-auto bg-gray-400 rounded-full p-3.5'><PersonIcon /></div>}
          ...
          <section className='px-2'>
            ...
            <div className='flex space-x-2 mt-2'>
              <button className='w-full bg-zinc-900 text-base' onClick={() => nav('/edit-profile')}>Edit profile</button>
              <button className='bg-zinc-900 p-1' onClick={toggleDiscoverUsers}><PersonPlusIcon /></button>
            </div>
            {profile.isShop ? <button onClick={() => nav(`/shop/${profile.username}`)} className='flex items-center justify-center space-x-2 mt-2 w-full bg-zinc-900 p-1'><ShopIcon className='w-5 h-5' /> <span className='text-base'>View shop</span></button> : null}
          </section>

          <section className='p-2'>
            {discoverUsers.length > 0 ? <div className='w-full py-3'>Discover people</div> : null}

            <div className='flex space-x-2'>
              {discoverUsers.map((discoverUser, index) => (
                <div key={index} className='flex flex-col border w-fit items-center overflow-scroll'>
                  <div className='flex flex-col p-4 items-center'>
                    <img className='aspect-square w-[75px] rounded-full' src={`http://127.0.0.1:8000${discoverUser.image}`} />
                    <span className='font-semibold'>{discoverUser.username}</span>
                  </div>
                  <div className='flex flex-col items-center w-4/5'>
                    <button className='p-2 mb-2 bg-blue-500 w-full rounded-md'>Follow</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className='px-2 py-6'>
            ...
            <section className='flex items-center'>
              <div className='flex border-b-2 w-1/2 py-1' ref={postsGridBtn} onClick={togglePostsGrid}>
                <GridIcon className='mx-auto' />
              </div>

              <div className='flex w-1/2 py-1' ref={taggedGridBtn} onClick={toggleTaggedGrid}>
            ...
            <section className='grid grid-flow-row grid-cols-3 md:grid-cols-4  gap-px overflow-scroll pb-14'>
              {postsGrid.map((post, index) => (
                <img key={index} className='h-24 w-full object-cover' src={`http://127.0.0.1:8000/media/${post.photoes}`}
