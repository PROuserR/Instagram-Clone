    ...
    return (
      <div>
        <section className='flex items-center'>
          <div onClick={() => nav('/')}><XIcon /></div>
          <span className='ml-10 text-xl font-semibold text-black dark:text-white'>New post</span>
          <div className='ml-auto mr-2 text-blue-700' onClick={addPost}>
            <CheckIcon />
          </div>
        </section>
        <input ref={caption} className='bg-transparent w-full h-10 mx-2 mt-10 outline-none' placeholder='Write a caption...' />
        <NavBar />
      </div>
    )
  else
    return (
      <div>
        ...
        <section className='fixed space-y-8 flex flex-col w-screen top-36 left-0 items-center'>
          <div className='text-xl text-black dark:text-white'>Create New Post</div>
          <div><ImagesIcon /></div>
          <input onInput={addFiles} ref={upload} type='file' hidden multiple />
          <button onClick={() => upload.current.click()} className='text-lg bg-blue-600 p-1 rounded-md text-black dark:text-white'>Select from device</button>
        </section>
        <NavBar />
      </div>
    )
