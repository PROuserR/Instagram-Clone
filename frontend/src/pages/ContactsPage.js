...
const ContactsPage = () => {
  ...
  const [onlineUsersContext, setOnlineUsersContext] = useContext(OnlineUsersContext)
  ...
  const getFollowedUsers = async () => {
    const res = await fetch(`http://127.0.0.1:8000/api/list_followed_users/${localStorage.getItem('my_id')}`)
    ...
    <div>
      <div className='p-2'>
        <section className='sticky top-0 left-2 bg-white dark:bg-black flex items-center py-2 space-x-4 z-10'>
          ...
          <span className='text-lg font-semibold'>{localStorage.getItem('my_username')}</span>
        ...
        <section className='py-4'>
          <div className='flex bg-zinc-800 py-1 px-3 rounded-lg'>
            ...
            <input ref={searchQuery} onChange={search} className='ml-3 text-base outline-none bg-transparent w-3/4' placeholder={'Search'} />
          </div>
        </section>
          ...
          {onlineUsersContext.map((onlineUser, index) => (
            <div key={index} className='flex space-x-4 overflow-x-scroll'>
              {onlineUser.id !== localStorage.getItem('my_id') ?
                <div onClick={() => nav(`/contact/${onlineUser.id}`)} className='flex flex-col items-center'>
                  <img className='rounded-full w-12 h-12' src={`http://127.0.0.1:8000/media/${onlineUser.profile_picture}`} />
          ...
          {followedUsers.map((followedUser, index) => (
            <div key={index} onClick={() => nav(`/contact/${followedUser.id}`)}>
              <div className='flex space-x-3'>
                <div>
                  <img className='rounded-full w-12 h-12' src={`http://127.0.0.1:8000/media/${followedUser.profile_picture}`} />
                </div>
                <div className='flex flex-col'>
                  <span className='mt-1'>{followedUser.username}</span>
                  <span className='text-gray-400 text-xs'>Active {timeDelta(followedUser.last_seen)}</span>
                </div>
              </div>
            </div>
          ))}
