...
const ProfilePage = () => {
    ...
    const getProfile = async () => {
        const res = await fetch(`http://127.0.0.1:8000/api/get_profile/${profile_id}`, {
            headers: {
        ...
        setProfile([data])
        setIsUserFollowed(data.followers.includes(parseInt(localStorage.getItem('my_id'))))
    }

    const followUser = async () => {
        setIsUserFollowed(true)
        await fetch(`http://127.0.0.1:8000/api/follow_user/${localStorage.getItem('my_id')}/${profile[0].user}`, {
            method: 'POST',
            headers: {
                'Authorization': `Token ${localStorage.getItem('token')}`
            }
        ...
        await fetch(`http://127.0.0.1:8000/api/add_activity/`, {
            ...
            body: JSON.stringify({ 'action': 'Started following you', 'influncer': localStorage.getItem('my_id'), 'influnced': profile[0].user })
        })
    }

    const unfollowUser = async () => {
        setIsUserFollowed(false)
        await fetch(`http://127.0.0.1:8000/api/unfollow_user/${localStorage.getItem('my_id')}/${profile[0].user}`, {
            method: 'POST',
            headers: {
                'Authorization': `Token ${localStorage.getItem('token')}`
            }
    ...
    const getPostsGrid = async () => {
        const res = await fetch(`http://127.0.0.1:8000/api/list_user_posts/${profile_id}`, {
            headers: {
                'Authorization': `Token ${localStorage.getItem('token')}`
            }
        });
            ...
            {profile.map((profile, index) => (
                <div key={index}>
                    <section className='flex items-center p-2'>
                        ...
                        <ListIcon className='w-8 h-8 ml-auto' onClick={() => setSettingsMenuFlag(!settingsMenuFlag)} />
                        {loginMenuFlag ?
                            <div className='flex flex-col space-y-4 p-4 pt-6 w-screen max-w-4xl bg-white dark:bg-zinc-900 fixed left-0 bottom-10 xl:left-72 z-10'>
                                ...
                                <div className='flex items-center space-x-2'>
                                    <div className='border border-gray-500 rounded-full p-1.5'>
                                        <PlusIcon className='w-8 h-8 cursor-pointer' onClick={() => nav('/addPost')} />
                                    </div>
                                    <span className='text-lg font-semibold' onClick={() => nav('/login')}>Add account</span>
                                </div>
                            </div>
                            : null}
                    ...
                    <section className='p-3 flex items-center'>
                        <img className='rounded-full w-[75px]' src={`http://127.0.0.1:8000${profile.image}`} />
                    ...
                    <section className='px-2'>
                        ...
                        <div className='flex space-x-2 mt-2'>
                            {isUserFollowed ? <button className='w-full bg-zinc-900' onClick={unfollowUser}>Following</button>
                                : <button className='w-full bg-zinc-900' onClick={followUser}>Follow</button>}
                            <button className='w-full bg-zinc-900' onClick={() => { }}>Message</button>
