...
const LoginPage = () => {
    ...
    useEffect(() => { loginBtn.current.disabled = true }, [])
    ...
    const socialLogin = async () => {
        const res1 = await fetch(`http://127.0.0.1:8000/auth/generate_token/${localStorage.my_username}`)
        const data1 = await res1.json()
        ...
        if (localStorage.socialLoggedFlag === 'true') {
            ...
            const file = new File([blob], `facebook-profilePicture-${localStorage.my_username}.jpg`)
    ...
    return (
        <div className='flex flex-col xl:my-[20%] my-[35%] items-center text-center space-y-4 p-2'>
            ...
            <input ref={username} className='w-5/6 p-2 bg-zinc-800 rounded-md outline-none' placeholder='Username' type='text' onChange={checkInputs} />
            <input ref={password} className='w-5/6 p-2 bg-zinc-800 rounded-md outline-none' type='password' placeholder='Password' onChange={checkInputs} />
            <button ref={loginBtn} className={`w-5/6 ${loginBtnColor} p-2`} onClick={loginUser}>Login</button>
