    ...
    <div>
      ...
      <section className='p-2'>
        {users.map((user, index) => (
          <div className='flex items-center mb-2' onClick={() => nav(`/profile/${user.id}`)} key={index}>
            <img className='w-12 h-12 rounded-full' src={`http://127.0.0.1:8000/media/${user.profile_picture}`} />
      ...
      <section className='grid grid-cols-3 md:grid-cols-4 gap-px overflow-y-scroll pb-14'>
        {discoveredPosts.map((discoveredPost, index) => (
          <div className='cursor-pointer' key={index} onClick={() => nav(`/post/${discoveredPost.id}`)}>
            <img src={`http://127.0.0.1:8000/media/${discoveredPost.photoes}`} />
          </div>
        ))}
      </section>
      <NavBar />
