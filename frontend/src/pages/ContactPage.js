...
const ContactPage = () => {
    ...
    const [imageBtn, setImageBtn] = useState(<div onClick={sendImage}><ImageIcon className='ml-auto' /></div>)
    ...
    const listChat = async () => {
        const res = await fetch(`http://127.0.0.1:8000/chat/list_chat/${localStorage.getItem('my_id')}/${user_id}`)
    ...
    const handleMessage = () => {
        if (message.current.value) {
            setSendBtn(<button className='ml-4 text-blue-600 text-lg' onClick={() => sendMessage(false)}>Send</button>)
            setImageBtn()
        }
        else {
            setSendBtn()
            setImageBtn(<div onClick={sendImage}><ImageIcon className='ml-auto' /></div>)
        }
    }
    ...
    useEffect(() => {
        ...
        chatSocket.onmessage = function (e) {
            ...
            temp_messages.push({
                ...
                'peer_id': data.peer_id, 'my_id': data.my_id
            })
    ...
    return (
        <div className='h-screen p-2'>
            <section className='static top-0 flex items-center w-full bg-white dark:bg-black space-x-4'>
                ...
                <img className='rounded-full w-9 h-9' src={`http://127.0.0.1:8000${userProfile.image}`} />
                <div className='flex flex-col'>
                    ...
                    <span className='text-base text-gray-400'>Active {timeDelta1(userProfile.last_login)}</span>
                </div>
            ...
            <section ref={chatList} className='my-4 space-y-6 overflow-scroll h-4/5'>
                {messages.map((message, index) => (
                    <div key={index}>
                        <div className='flex'>
                            <div className='mx-auto text-gray-500'>{index > 1 && timeDelta2(messages[index].date_added, messages[index - 1].date_added).includes('h') ? getInstaDate(message.date_added) : null}</div>
                        </div>
                        {parseInt(message.my_id) === parseInt(localStorage.getItem('my_id')) ?
                            <div className='flex flex-col items-center'>
                                {message.content ? <span className='ml-auto mr-2 p-2 bg-blue-600 rounded-xl'>{message.content}</span> : null}

                                {message.image ? <img className='w-1/2 rounded ml-auto' src={`http://127.0.0.1:8000${message.image}`} /> : null}
                            </div>
                            :
                            <div className='flex items-center'>
                                <img className='rounded-full w-9 h-9' src={`http://127.0.0.1:8000${userProfile.image}`} />
                                {message.content ? <span className='ml-4 p-2 bg-blue-600 rounded-xl'>{message.content}</span> : null}

                                {message.image ? <img className='w-1/2 rounded ml-4' src={`http://127.0.0.1:8000${message.image}`} /> : null}
                            </div>
                        }
                    </div>
                ))}
            ...
            <section className='mb-4 fixed bottom-0 w-full max-w-4xl z-10'>
                <div className='flex items-center mr-4 bg-white dark:bg-zinc-900 p-1 border dark:border-none rounded-lg'>
                    <input ref={message} onChange={handleMessage} onKeyDownCapture={sendMessageByEnter} className='w-[90%] outline-none bg-transparent p-1 text-black dark:text-white' placeholder='Message..' />
