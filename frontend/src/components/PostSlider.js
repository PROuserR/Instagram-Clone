    ...
    const showHeart = () => {
        ...
        if ((times[times.length - 1] - times[times.length - 2]) / 1000 < 0.3 && clickCounter >= 2) {
            if (showHeartFlag === false)
                setBigFilledHeartIcon(<BigFilledHeartIcon className='fixed pulsing-heart left-[40%] bottom-1/2 z-10' />)
            else
                setBigFilledHeartIcon(null)
            ...
            <div onClick={showHeart}>
                <Swiper
                    ...
                    <SwiperSlide className='h-64'>
                        <video ref={video} onClick={handlePlayMute} width="100%" src="https://s3-us-west-2.amazonaws.com/converterpoint-22/encodings/66f8c67e14f782d829aa93a191d5b4ea.mp4" autoPlay muted loop>
                        </video>
                ...
                <Swiper
                    ...
                    {photoes.map((photo, index) => (
                        <SwiperSlide key={index}>
                            <img className='pb-4 mx-auto' src={`http://127.0.0.1:8000/media/${photo}`} />
                            {bigFilledHeartIcon}
